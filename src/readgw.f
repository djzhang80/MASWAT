...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      character (len=13),intent(in) :: gwfile2
           character (len=13):: gwfile
      character(len=:),allocatable:: string
      integer::hruk=0
      hruk=0
       gwfile=gwfile2(1:12)//'f'
      eof = 0
      hlife_ngw = 0.0

      do
        string=getstr(gwfile,hruk,s_count)
      read (string,5000) titldum
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)

      read (string,*) shallst(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) deepst(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) delay(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) alpha_bf(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) gwqmn(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) gw_revap(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) revapmn(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*) rchrg_dp(ihru)
        deallocate(string)
        string=getstr(gwfile,hruk,s_count)
      read (string,*,iostat=eof) gwht(ihru)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(gwfile,hruk,s_count)
      read (string,*,iostat=eof) gw_spyld(ihru)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(gwfile,hruk,s_count)
      read (string,*,iostat=eof) shallst_n(ihru)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(gwfile,hruk,s_count)
      read (string,*,iostat=eof) gwminp(ihru)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(gwfile,hruk,s_count)
      read (string,*,iostat=eof) hlife_ngw
        deallocate(string)
      if (eof < 0) exit
