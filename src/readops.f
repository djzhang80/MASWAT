...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use  fortran_calls_c
      !Implicit None
      character (len=13),intent(in) :: opsfile
	...
	real :: mgt3,mgt4,mgt5,mgt6,mgt7,mgt8,mgt9
      ...
      hruk=0
        ...
        string=getstr(opsfile,hruk,s_count)
         kkk=len(string)
         if(kkk==0) then
             goto 999   
             end if
	  ...
	  read (string,5000,end=999) titldum
        deallocate(string)
...
!!      read scheduled operations
        ...
        string=getstr(opsfile,hruk,s_count)
          read (string,5200,iostat=eof) mon, day, iyear, mgt_op, mgt1i,   &
