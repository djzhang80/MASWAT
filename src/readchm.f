...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      character (len=13),intent(in) :: chmfile
      ...
      do
        string=getstr(chmfile,hruk,s_count)
      read (string,5000,iostat=eof) titldum
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5000,iostat=eof) titldum
      ...
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5100,iostat=eof) (sol_no3(j,ihru), j = 1, mlyr)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5100,iostat=eof) (sol_orgn(j,ihru), j = 1, mlyr)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5100,iostat=eof) (sol_solp(j,ihru), j = 1, mlyr)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5100,iostat=eof) (sol_orgp(j,ihru), j = 1, mlyr)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5100,iostat=eof) (pperco_sub(j,ihru), j = 1, mlyr)
        deallocate(string)
      if (eof < 0) exit
        string=getstr(chmfile,hruk,s_count)
      read (string,5000,iostat=eof) titldum
...
!!	end do
      ...
      do j = 1, mpst
        ...
        pstenr = 0.
        string=getstr(chmfile,hruk,s_count)
        read (string,*,iostat=eof) pstnum, pltpst, solpst, pstenr
        deallocate(string)
        if (pstnum > 0) then
          hrupest(ihru) = 1
          newpest = 0
