...
!      real :: xx, plt_zmx, yy
      real :: plt_zmx                   !Claire, xx and yy are not used 12/2/09
       ...
       character (len=13),intent(in) :: solfile
...
!!    initialize remaining soil parameters      
      ...
      sol_silt = 0.
        ...
        string=getstr(solfile,hruk,s_count)
      read (string,5500) titldum
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5100) snam(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5200) hydgrp(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5300) sol_zmx(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5400) anion_excl(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5600) sol_crk(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5500) titldum
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_z(j,ihru), j = 1, mlyr)
        deallocate(string)
...
!      sol_z(j,ihru) = sol_z(j,ihru) / 5.0
      ...
      do
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_bd(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_awc(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_k(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_cbn(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_clay(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_silt(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_sand(j,ihru), j = 1, nly)
        deallocate(string)

        string=getstr(solfile,hruk,s_count)
      read (string,5000) (sol_rock(j,ihru), j = 1, nly)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) sol_alb(ihru)
        deallocate(string)
        string=getstr(solfile,hruk,s_count)
      read (string,5000) usle_k(ihru)
        deallocate(string)
!    change below double subscripted sol_ec statement 1/27/09 when making septic changes
        string=getstr(solfile,hruk,s_count)
      read (string,5000,iostat=eof) (sol_ec(j,ihru), j = 1, nly)
        deallocate(string)
!    change below double subscripted sol_ec statement 1/27/09 when making septic changes
      if (eof < 0) exit
      exit
